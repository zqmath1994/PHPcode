...
<div id="content">
	<!-- 顶部右侧快捷操作按钮 开始 -->
		...
		<div id="breadcrumb">
			...
			<a href="__URL__/bclass" title="返回分类管理" class="tip-bottom"> 分类管理</a>
			<a href="#" class="current"> 修改分类信息</a>
		</div>
		<!-- 顶部面包屑导航 结束 -->
		<!-- 主体区域 -->
				...
				<div class="span12">
					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							<!-- 编辑表单 -->
							<form action="__URL__/saveb/id/<?php echo ($data["id"]); ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
								...
								<div class="control-group">
									...
									<div class="controls"><input type="file" name="image"/><br /><img src="__PUBLIC__/images/<?php echo ($data["image"]); ?>" style="margin-top:10px"></div>
								</div>
								<div class="control-group">
									<label class="control-label">所属导航：</label>
									<div class="controls">
										<select name="pid" id="" style="width:100px;">
											<?php if(is_array($data["menu"])): foreach($data["menu"] as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
										</select>
									</div>
								</div>
								<!-- 提交提交按钮 -->
